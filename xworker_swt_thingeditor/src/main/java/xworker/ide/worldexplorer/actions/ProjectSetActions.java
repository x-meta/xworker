...
public class ProjectSetActions {
	private static final String TAG = ProjectSetActions.class.getName();
					...
					try{
						if(child.getBoolean("isThingRoot")){
							...
							if(world.getThingManager(name) == null){
								...
								Executor.info(TAG, "ThingManager <" + name + "> already exists");
							}								
						}else{
							world.initThingManager(file, child.getMetadata().getName());
						}
					}catch(Exception e){								
						Executor.warn(TAG, "ProjectSet add project error, file=" + file.getPath() 
								+ ",path=" + self.getMetadata().getPath(), e);
					}
				}
			}else{
				Executor.warn(TAG, "ProjectSet file not exists or is not a file, file=" + dir 
	...
	public static void addProject(ActionContext actionContext){
		...
		if(!have){
			...
			Executor.info(TAG, "Project file alrady exists.");
		}
	}
}
