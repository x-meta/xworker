...
public class ExplorerActions {
	private static final String TAG = ExplorerActions.class.getName();
	...
	public static void openThing(ActionContext actionContext) throws OgnlException{
		...
		if(forOpen == null){
		    Executor.info(TAG, "OpenThing: thing not exists, thingPath=" + self.getMetadata().getPath());
		}
	...
	public static void startBackgroudThread(ActionContext actionContext){
        ...
        while(tasks.hasNext()){
            ...
            try{
                ...
                t.printStackTrace();
                Executor.error(TAG, "run background task error", t);
            }
        }
		/*
		final World world = World.getInstance();
