...
public class DataObjectFormReadCreator {
	private static Logger log = LoggerFactory.getLogger(DataObjectFormReadCreator.class);
	...
	public static void doAction(ActionContext actionContext) throws IOException{
        ...
        if(dataObjectConfig == null){
            ...
            try{
                ...
                //通过关键字装载数据
                Thing keyThing = (Thing) theData.getKeyAndDatas()[0][0];
                theData.put(keyThing.getString("name"), request.getParameter("id"));
                //theData.doAction("parseHttpRequestData", actionContext);
                theData.doAction("load", actionContext);
