...
public class DocumentActions {
	...
	public static void body(ActionContext actionContext) {
		Thing self = actionContext.getObject("self");
		Document document = actionContext.getObject("document");

		for(Thing child : self.getChilds()) {
			child.doAction("run", actionContext, "element", document.body());
		}
	}
	
	public static void head(ActionContext actionContext) {
		Thing self = actionContext.getObject("self");
		Document document = actionContext.getObject("document");

		for(Thing child : self.getChilds()) {
			child.doAction("run", actionContext, "element", document.head());
		}
	}
	
	public static Document run(ActionContext actionContext) {
		Thing self = actionContext.getObject("self");
		
		Document document = self.doAction("createDocument", actionContext);
		for(Thing child : self.getChilds()) {
			child.doAction("run", actionContext, "document", document, "element", document);
		}
		
		return document;
	}
	
	public static Document createDocument(ActionContext actionContext) throws IOException {
		Thing self = actionContext.getObject("self");
