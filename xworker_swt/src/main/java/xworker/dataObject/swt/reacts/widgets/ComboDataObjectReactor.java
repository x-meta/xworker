...
public class ComboDataObjectReactor extends WidgetDataObjectReactor{
	Combo widget;
	
	public ComboDataObjectReactor(Combo widget, Thing self, ActionContext actionContext) {
		...
		this.widget.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				List<DataObject> dataObjects = DataStore.getWidgetRecords(ComboDataObjectReactor.this.widget);
				if(dataObjects != null) {
					if(event.index < dataObjects.size()) {
						DataObject dataObject = dataObjects.get(event.index);
						List<DataObject> datas = ComboDataObjectReactor.this.toDataObjectList(dataObject);
						ComboDataObjectReactor.this.fireSelected(datas);
					}
				}				
			}			
		});
	...
	public static void create(ActionContext actionContext) {
		...
		Combo widget = self.doAction("getCombo", actionContext);
		if(widget == null) {
		...
		if(widget != null) {
			ComboDataObjectReactor reactor = new ComboDataObjectReactor(widget, self, actionContext);
