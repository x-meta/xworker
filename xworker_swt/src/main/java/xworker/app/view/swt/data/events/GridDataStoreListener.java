...
public class GridDataStoreListener {
	...
	public static String getColumnDisplayValue(final ActionContext actionContext){
		...
		if(column.getData("__gridDataStoreToDisplayString") != null && ((Boolean) column.getData("__gridDataStoreToDisplayString")) == true){
		...
		}else if(column.getData("__gridDataStoreFormater") != null){ 
		    Format formater = (Format) column.getData("__gridDataStoreFormater");
	...
	public static void createRowEditor(final ActionContext actionContext){
		...
		Thing cursorThing = new Thing("xworker.swt.nubula.Grid/@GridColumnEditor");
		...
		/*
		//默认使用Attribute编辑器
		def defaultEditor = new Thing("xworker.swt.custom.GridCursorEditor/@ColumnEditor");
		defaultEditor.name = "defaultEditor";
		cursorThing.addChild(defaultEditor);
		//默认是文本编辑
		def textEditor = new Thing("xworker.swt.custom.GridCursorEditor/@ColumnEditor/@TextEditor");
		...
		*/
		...
		for(Thing column : columns){
		    if(column.getBoolean("editable")){
		        ...
		        Thing columnEditor = new Thing("xworker.swt.nubula.Grid/@GridColumnEditor/@ColumnEditor");
		...
		try{
		    ...
		    bindings.put("parent", (Grid) actionContext.get("grid"));
	...
	public static void onRemove(final ActionContext actionContext){
		...
		final Grid grid = (Grid) self.get("grid");
		...
		Thing store = (Thing) actionContext.get("store");
		SwtStoreUtils.runAsync(store, grid.getDisplay(), new Runnable(){
			public void run(){
				try{
					DSSelectionListener.removeRecords(grid, records);
			        ...
			        for(DataObject record : records){
			            for(GridItem item : grid.getItems()){
			                ...
			                if(record == itemRecord){
			                    ...
			                    if((grid.getStyle() & SWT.CHECK) == SWT.CHECK){
	...
	public static void beforeLoad(final ActionContext actionContext){
		...
		Thing store = (Thing) actionContext.get("store");
		SwtStoreUtils.runSync(store, grid.getDisplay(), new Runnable(){
			public void run(){
			    try{
			        //先清空数据
			        grid.removeAll();
	...
	public static void onLoaded(final ActionContext actionContext){
		...
		Runnable runner = new Runnable(){
			...
			public void run(){
			    try{
			    	DSSelectionListener.clear(grid);
		...
		SwtStoreUtils.runAsync(store, grid.getDisplay(), runner);
	}
	...
	public static void onReconfig(final ActionContext actionContext){
		...
		SwtStoreUtils.runSync(store, grid.getDisplay(), new Runnable(){
			...
			public void run(){
				try{
					DSSelectionListener.bind(grid, store, actionContext);
			        grid.setData("_store", store);

			        //删除所有数据
			        if(grid.getItemCount() > 0) {
			        	grid.remove(0, grid.getItemCount());
			        }
			        //grid.removeAll();
			        //删除所有列
			        for(GridColumn column : grid.getColumns()){
			        ...
			        if(defaultSelectionListener != null){
			            grid.removeListener(SWT.DefaultSelection, defaultSelectionListener);
			        }
			        Listener selectionListener = (Listener) self.get("selectionListener");
			        if(selectionListener != null){
			            grid.removeListener(SWT.Selection, selectionListener);
			        }
			        
			        //删除GridCursor和相关Listener
			        Control gridCursor = (Control) grid.getData("tableCursor");
			        if(gridCursor != null){
			        	gridCursor.dispose();
			        }
			        Listener gridCursorListener = (Listener) grid.getData("gridCursorListener");
			        if(gridCursorListener != null){
			        	grid.removeListener(SWT.Selection, gridCursorListener);
			        }
			        ...
			        Map<String, GridColumnGroup> groups = new HashMap<String, GridColumnGroup>(); 
			        ...
			        for(Thing attr : (List<Thing>) dataObject.get("attribute@")){
			            ...
			            if(attr.getBoolean("viewField") && attr.getBoolean("showInTable") && attr.getBoolean("gridEditor")){
			            	GridColumnGroup gridColumn = null;
			            	String columnGroup = attr.getStringBlankAsNull("columnGroup");
			            	if(columnGroup != null) {
			            		gridColumn = groups.get(columnGroup);
			            		if(gridColumn == null) {
			            			gridColumn = new GridColumnGroup(grid, SWT.NONE | SWT.CENTER);
			            			gridColumn.setText(columnGroup);
			            			groups.put(columnGroup, gridColumn);
			            		}
			            	}
			            	...
			            	if(gridColumn != null) {
			            		column = new GridColumn(gridColumn, style);
			            	}else {
			                ...
			                column.setResizeable(attr.getBoolean("gridResizable"));
			                ...
			                if(attr.getActionThing("toDisplayString") != null){
			                    attr.setData("__gridDataStoreToDisplayString", true);
			                }else{
			                    attr.setData("__gridDataStoreToDisplayString", false);
			                }
			                //pattern
			                ...
			                if(pattern != null){
			                	...
			                	if(UtilData.equalsOne(type, new String[]{"date", "datetime", "time"})){
			                		attr.setData("__gridDataStoreFormater", new SimpleDateFormat(pattern));
			                	}else if(UtilData.equalsOne(type, new String[]{"byte", "short", "int", "double", "float"})){
			                	    attr.setData("__gridDataStoreFormater", new DecimalFormat(pattern));
			                    }
			                }
			                columns.add(attr);
			            }
			        ...
			        if(editable){
			            ...
			            if("window".equals(editMethod)){
			                ...
			                Thing listenerThing = world.getThing("xworker.app.view.swt.data.events.GridDataStoreListener/@listeners/@tableDefaultSelection/@tableEditAction1");
			                SwtListener listener = new SwtListener(listenerThing, actionContext, true);
			                grid.addListener(SWT.DefaultSelection, listener);
			                self.put("defaultSelectionListener", listener);    
			            }else{
			                //创建表格编辑
			                self.doAction("createRowEditor", actionContext, UtilMap.toMap(new Object[]{"columns", columns, "grid", grid, "store", store}));
			            }
			        }
			        //添加选择事件
			        Thing listenerThing = world.getThing("xworker.app.view.swt.data.events.GridDataStoreListener/@listeners/@tableSelection/@tabelSelectionAction");
			        SwtListener listener = new SwtListener(listenerThing, actionContext, true);
			        grid.addListener(SWT.Selection, listener);
	...
	public static void gridEditAction(ActionContext actionContext){
		...
		Thing store = (Thing) grid.getData("_store");
		ac.put("store", store);
		ac.put("parent", grid.getShell());
	...
	public static void gridSelectionAction(ActionContext actionContext){
		Event event = (Event) actionContext.get("event");
