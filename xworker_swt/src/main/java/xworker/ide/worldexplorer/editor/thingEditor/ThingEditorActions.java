...
public class ThingEditorActions {
	...
	//	log.info("open thing");
		...
		if(treeThing == currentThing && !UtilData.isTrue(actionContext.get("forceOpen"))){
		    ...
		    if(!UtilData.isTrue(actions.doAction("save"))){
		         return ;
		    }
		}
		...
		if(menuBarComposite != null && !menuBarComposite.isDisposed()){
			...
			List<Thing> childComboDatas = new ArrayList<>(structures);
	...
	public Object modify(ActionContext actionContext) throws OgnlException {
		...
		if(actionContext.get("titleLabel") != null){    
			...
			if(!titleLabel.isDisposed()) {
			    ...
			    if(UtilData.isTrue(actionContext.get("modified"))){
	...
	public Object initOutlineBrowser(ActionContext actionContext) {
		...
		//类和描述者
		Map<String, Thing> dess = new HashMap<>();
		...
		//继承
		Map<String, Object> exss = new HashMap<>();
	...
	public Object initOutlineThingActions(ActionContext actionContext) {
		...
		Map<String, String> acss = new HashMap<>();
		Map<String, String> acds = new HashMap<>();
...
//		    actionContext.printStackTrace();
	...
	public Object focusAttributeInput(ActionContext actionContext) {
		String name = actionContext.getObject("name");
		Executor.info(TAG, "name=" + name + "Input");
		Object c = actions.doAction("getAttributeInput");
		Executor.info(TAG, "forcus c=" + c);
	...
	public Object descriptComboSelection(ActionContext actionContext) {
		...
		if(menuBarComposite != null && !menuBarComposite.isDisposed()){
		    ...
		    if(objStruct == null){
		    	...
		    	thingMenus = ThingMenu.getThingMenus(currentThing, objStruct, "swt", actionContext);
			}
		...
		//切换到默认编辑方式
		String defaultEditor = objStruct.getString("defaultEditor");
	...
	public Object refreshMenuSelection(ActionContext actionContext) {
		...
		if(UtilData.isTrue(actionContext.get("disableOpenThingAfterRefresh"))){
	...
	public Object showAddChildSelectionListener(ActionContext actionContext) {
		...
		Listener descriptsComboSelection = actionContext.getObject("descriptsComboSelection");
		if(!UtilData.isTrue(actionContext.get("doInitChildTree"))){
		    //如果子节点树没有初始化，那么初始化
		    actionContext.getScope(0).put("doInitChildTree", true);
	...
	public Object initCode(ActionContext actionContext) {
		...
		if(dialogThing == null){
		    ...
		    ((Widget) actionContext.get("commandButton")).dispose();
			((Widget) actionContext.get("assistorItem")).dispose();
		}
		return null;
	}
	...
	public void doAction(ActionContext actionContext){
		...
		if(innerOutline != null && !innerOutline.isDisposed()){
	...
	public Map<String, Object> getValues(ActionContext actionContext){
		MessageCenter.publish("/xworker/ide/thingeditor/getvalues", null, actionContext);

		Thing currentModel = actionContext.getObject("currentModel");
		ActionContext currentModelContext = actionContext.getObject("currentModelContext");

		return currentModel.doAction("getValue", currentModelContext);
	}
