...
public class DataTreeTreeEditorCreator {
    public static void toJavaCode(ActionContext actionContext){
    	/*
        String code = "${self.name} = new TreeEditor(${self.parent.parent.name});";
    ...
    public static void toEditorResetCode(ActionContext actionContext){
        ...
        for(child in self.childs){
            ...
            if(scriptObj != null){
                ...
                code = code + "\n    oldItem.setText(${self.detachedSource.parent.getItemIndex("TreeColumn")}, ${che.name}.getText());";
            }
            //只处理一个子节点
            break;
        }    
