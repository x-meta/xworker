...
public class RectangleEditorCreator {
    public static Object create(ActionContext actionContext){
		...
		//输入编辑器
		Thing inputThing = world.getThing("xworker.swt.xworker.attributeEditor.RectangleEditor/@composite");
    ...
    public static void editButtonAction(ActionContext actionContext){
    	...
    	Thing shellThing = world.getThing("xworker.swt.xworker.attributeEditor.RectangleEditor/@shell");
    	...
    	try{
    	    ...
    	    ((Text) actionContext.get("widthText")).setText(points[2]);
    	    
    	    Integer.parseInt(points[3]);
    	    ((Text) actionContext.get("heightText")).setText(points[3]);
    	}catch(Exception e){
    	}
    	shell.open();
    }
	...
	public static void okButtonSelection(ActionContext actionContext){
    	...
    	shell.dispose();
    	if(data.get("x") != null && data.get("y") != null && data.get("width") != null && data.get("height") != null){
    	    pointText.setText("" + data.get("x") + "," + data.get("y") + "," + data.get("width") + "," + data.get("height"));
    	    pointText.setFocus();
    	}
    }
