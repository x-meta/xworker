...
public class DateEditorCreator {
    public static Object create(ActionContext actionContext){
		...
		if(actionContext.get("isThingEditor") == null){
		    ...
		    actionContext.getScope(0).put(self.getString("name"), composite.getData(AttributeEditor.INPUT_CONTROL));
		}else{
		    //在事物编辑器里被调用
		    ...
		    //Text输入框为输入属性编辑器的Model事物，直接返回text出现不了日期选择器
		    actionContext.getScope(0).put(attribute.getMetadata().getName() + "Input", text);
		}
