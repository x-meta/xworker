...
public class AnalogClock24Drawer extends AbstractDrawer implements ClockDrawer {
	
	public AnalogClock24Drawer(Thing thing, ActionContext actionContext) {
	...
	public void drawClock(Date date, float x, float y, float width, float height,  GC gc) {
		...
		Color color12 = new Color(gc.getDevice(), 0x66, 0x66, 0x66);	
		...
		try {
			...
			for ( int i=1; i<= 24; i++){
			    ...
			    // Get the angle from 12 O'Clock to this tick (radians)
			    double angleFrom12 = di/120.0*10.0*Math.PI;
	        ...
	        double angle = (hour/24.0)*360 - 180 ;
	        ...
	        if(showHour) {
		        ...
		        hourHand.lineTo((float)0-2, (float)((r-longTickLen)*0.75));
		        hourHand.lineTo((float)0-5, (float)0);
		        hourHand.close();
	        ...
	        for ( int i=1; i<=24; i++){
			    ...
			    // be drawn
			     // Get the angle from 12 O'Clock to this tick (radians)
			    double angleFrom12 = di/24.0*2.0*Math.PI;
	...
	public static AnalogClock24Drawer create(ActionContext actionContext) {
		Thing self = actionContext.getObject("self");
		return new AnalogClock24Drawer(self, actionContext);
	}
