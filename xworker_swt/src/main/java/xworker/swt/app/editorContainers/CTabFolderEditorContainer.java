...
public class CTabFolderEditorContainer extends AbstractEditorContianer
	...
	public CTabFolderEditorContainer(CTabFolder tabFolder, ActionContext actionContext) {
		...
		Thing containerThing = World.getInstance()
				.getThing("xworker.swt.app.prototypes.CTabFolderEditorContainer/@cTabFolder");
...
//		}
	...
	public static void actions_close(ActionContext actionContext) {
		CTabItem item = actionContext.getObject("item");
		IEditor editor = actionContext.getObject("editor");
		CTabFolderEditorContainer container = actionContext.getObject("container");
		Action confirm = actionContext.getObject("confirm");

		if (editor != null && editor.isDirty()) {
			confirm.run(actionContext);
		} else {
			container.doClose(item);
		}
	}

	public static void actions_close_yes(ActionContext actionContext) {
		CTabItem item = actionContext.getObject("item");
		IEditor editor = actionContext.getObject("editor");
		CTabFolderEditorContainer container = actionContext.getObject("container");
	...
	public static void actions_close_cancel(ActionContext actionContext) {
		Event event = actionContext.getObject("event");
		event.doit = false;
	}

	public static void cnTabMenuSelection(ActionContext actionContext){
        //xworker.swt.app.prototypes.CTabFolderEditorContainer/@cTabFolder/@centerTabFolderMenu/@cn_closeSelfItem/@listeners/@cnTabMenuSelection/@GroovyAction
		Event event = actionContext.getObject("event");
		CTabFolder cTabFolder = actionContext.getObject("cTabFolder");
        if(event.widget == actionContext.get("cn_closeSelfItem")){
            CTabItem item = cTabFolder.getSelection();
            disposeItem(item);
            item.dispose();
        }else if(event.widget == actionContext.get("cn_closeOtherItem")){
        	CTabItem item = cTabFolder.getSelection();
            for(CTabItem it : cTabFolder.getItems()){
                if(it != item){            
                    disposeItem(it);
                }
            }
        ...
        }else if(event.widget == actionContext.get("cnCloseAllItem")){
	...
	public static void disposeItem(CTabItem item){
        //销毁CtabItem相关的内容
        ...
        Tree outlineTree = (Tree) item.getData("outlineTree");
        Composite childComposite = (Composite) item.getData("childComposite");
