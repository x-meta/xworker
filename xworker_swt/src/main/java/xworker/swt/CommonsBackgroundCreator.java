...
public class CommonsBackgroundCreator {
    public static void create(ActionContext actionContext) throws OgnlException{
        ...
        if(color != null){
            ((Control) actionContext.get("parent")).setBackground(color);
        }else{
            Thing colorObj = self.getThing("Color@0");
            if(colorObj != null){
                ActionContext newContext = new ActionContext();
                newContext.put("parent", null);
                Object obj = colorObj.doAction("create", newContext);
